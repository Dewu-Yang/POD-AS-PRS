...
def finite_difference_gradients_forward(X, fun, h=1e-6):
...
def finite_difference_gradients(X, fun, h=1e-6):
    """Compute finite difference gradients with a central difference scheme.
    ...
    # Build forward and backward perturbations for each dimension (2*m points total)
    e = np.eye(m)
    XX_plus  = np.kron(np.ones((m, 1)), X) + h * np.kron(e, np.ones((M, 1)))
    XX_minus = np.kron(np.ones((m, 1)), X) - h * np.kron(e, np.ones((M, 1)))

    # Stack all perturbed points and run the simulation
    XX = np.vstack((XX_plus, XX_minus))
    if isinstance(fun, SimulationRunner):
        F = fun.run(XX)
    else:
        F = SimulationRunner(fun).run(XX)

    # Extract forward and backward function values, then apply central difference
    F_plus  = F[:m * M].reshape((m, M))
    F_minus = F[m * M:].reshape((m, M))
    df = (F_plus - F_minus).transpose() / (2.0 * h)
