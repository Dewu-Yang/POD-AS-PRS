...
from .utils.rs import PolynomialApproximation
...
class Subspaces():
    ...
    def compute(self, X=None, f=None, df=None, weights=None, sstype='AS', ptype='EVG', nboot=0):
        ...
        if ptype == 'EVG':
            n, ediff = eig_partition(e)
        elif ptype == 'RS':
            sub_err = sub_br[:,1].reshape((m-1, 1))
            n, errbnd = errbnd_partition(e, sub_err)
        elif ptype == 'LI':
            n, G = ladle_partition(e, li_F)
        else:
...
class SnapshotSubspaces(Subspaces):
    """Memory-efficient active subspace class for large-scale data using the snapshot method.

    Instead of assembling the full m-by-m covariance matrix C = df^T df, this
    class works with the M-by-M Gram matrix K = df df^T, which is cheaper when
    M << m (many features, few samples).
    """

    def compute(self, df, n_components=10):
        """Compute the active subspace via the snapshot method.

        Parameters
        ----------
        df : ndarray
            M-by-m gradient matrix (M samples, m features).
        n_components : int
            Target dimension of the active subspace (default 10).
        ...
        print(f"Snapshot method: M={M} samples, m={m} features")

        # Step 1: build the M-by-M Gram matrix K = df df^T
        K = np.dot(df, df.T)

        # Step 2: eigen-decomposition of K
        eigenvals_K, eigenvecs_K = np.linalg.eigh(K)
        ...
        # Step 4: recover only the leading n_components eigenvectors of C
        n = min(n_components, min(M, m))
        scaling = np.diag(1.0 / np.sqrt(eigenvals_K[:n] + 1e-10))
        W1 = np.dot(df.T, np.dot(eigenvecs_K[:, :n], scaling))

        # Step 5: orthogonalise via QR
        W1, _ = np.linalg.qr(W1)

        # Step 6: store only the necessary data
        self.eigenvals = eigenvals_K[:m]  # at most m eigenvalues
        self.eigenvecs = None             # full eigenvector matrix not stored
        self.W1 = W1                      # leading n_components directions
        self.W2 = None                    # inactive subspace not stored

        print(f"Active subspace computed: dimension={n}, eigenvalues shape={self.eigenvals.shape}")
        return self
